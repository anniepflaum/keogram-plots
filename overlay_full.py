...
def process_date(date_str: str):
    ...
    ace_file  = next(ACE_DIR.glob(f"*dscovr_s{year}{month}{day}*pub.nc.gz"), None)

    if not all([keo_file, goes_file, ace_file]):
        print(f"Missing data for {date_str}: ")
        print(f"  keogram: {keo_file}, GOES: {goes_file}, ACE: {ace_file}")
        return
    ...
    # ACE data: handle potential EOFError on truncated gzip
    try:
        with gzip.open(ace_file, 'rb') as gz, tempfile.NamedTemporaryFile(delete=False, suffix='.nc') as tmp:
            shutil.copyfileobj(gz, tmp)
            tmp_path = tmp.name
    except (OSError, EOFError) as e:
        print(f"Error reading ACE file for {date_str}: {e}")
        return

    ds_ace = Dataset(tmp_path, 'r')
    ace_time = num2date(
        ds_ace.variables['time'][:], ds_ace.variables['time'].units,
    ...
    # Prepare DataFrame for ACE Bz
    ace = (
        pd.DataFrame({'time': ace_time, 'bz': bz})
        .dropna()
    ...
    ace['hour'] = (ace.index - pd.Timestamp(f'{year}-{month}-{day}')).total_seconds() / 3600
    ace = ace[(ace['hour'] >= first_h) & (ace['hour'] <= last_h)]
